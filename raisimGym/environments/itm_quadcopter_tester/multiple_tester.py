...
if weight_path == "":
    ...
    for i in range(int((3000-500)/50)):
        print('----------------------------------------------------')
        print('{:<40} {:>6}'.format("Agent: ", '{:0.10f}'.format(iteration_number + i * 50)))
        loaded_graph = module.MLP(cfg['architecture']['policy_net'], eval(cfg['architecture']['activation_fn']), ob_dim_learner, act_dim)
        loaded_graph.load_state_dict(torch.load(weight_path.rsplit('/', 1)[0]+"/full_"+str(iteration_number + i * 50)+'.pt')['actor_architecture_state_dict'])
        ...
        all_dones.append(num_failed_envs)
        new_index.clear()

np.savetxt(os.path.join(weight_dir, 'all_agent_rewards.csv'), all_rewards, delimiter=",")
np.savetxt(os.path.join(weight_dir, 'all_agent_failed_env.csv'), all_dones, delimiter=",")
