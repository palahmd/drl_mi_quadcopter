...
namespace raisim {
    ...
    public:
        ...
        float step(const Eigen::Ref<EigenVec> &action) final {
            ...
            /// set target sequence with visualization points
            /*
            if (targetLoopCount_== 800) setTarget(0, 20, -5);
            if (targetLoopCount_== 1600) setTarget(-10, 10, -7.5);
            if (targetLoopCount_== 2400) setTarget(0, 0, -10);
            if (targetLoopCount_== 3200) setTarget(10, 10, -12.5);
            */
        ...
        void applyThrusts() {
            ...
            /// when quadcoper hits the ground: stop moving
            if(hitGround) genForces_ *= 0;
            robot_->setGeneralizedForce(genForces_);

            /// set a side hit
            if(targetLoopCount_ == 250 ) {
        ...
        void setState(double pos, double q1, double q2, double q3, double q4, double vel, double angVel){
            for (int i=0; i<3; i++){
                gc_init_(i) = 1;
                gv_init_(i) = 1;
                gv_init_(i+3) = 1;
